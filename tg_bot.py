...
class GetTasks:
    def __init__(self):
        self.db_querry = DBQuerry()

    async def get_task_main(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        keyboard = [
    ...
    async def get_task_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            if query.data == 'request_applications':
                ...
                formatted_data = self.get_task_data(data, user_id, query.data)
    ...
    def get_task_data(self, data, user_id, application_type):
        formatted_strings = []
        items = []

        last_task_date_stored = self.db_querry.get_last_task_date(user_id, application_type)
        ...
        for task_id, contract, formatted_date, comment in sorted_items:
            ...
            task_status = self.db_querry.get_task_status_and_assignees(task_id=task_id)
        ...
        self.db_querry.update_task_date(user_id, application_type, latest_date_current)
...
class TakeTasks:
    ...
    async def take_task(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        keyboard = [
    ...
    async def take_task_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query
        await query.answer()
        ...
        if query.data in ['take_faults', 'take_install_with_cable', 'take_install_without_cable']:
            ...
            if task_address:
                existing_executors = self.db_querry.check_existing_task_in_progress(task_id)
                if existing_executors:
                    await query.edit_message_text(
                        f"Заявка {task_address} уже в работе исполнителями: {', '.join(existing_executors)}."
                    )
                else:
                    users = self.db_querry.get_users()
            ...
            if selected_executors:
                ...
                self.db_querry.update_task_status(task_id, selected_executors, "В работе")
                task_address = context.user_data.get("tasks", {}).get(task_id, [None])[0]
...
class CloseTaskMonitoring:
    def __init__(self):
        self.db_querry = DBQuerry()

    async def close_task_monitoring(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        groups = self.db_querry.get_work_groups()
    ...
    async def close_monitoring_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        ...
        if query.data.startswith("group_"):
            ...
            work_types = self.db_querry.get_work_type(group_id)
            ...
            employee_ids = self.db_querry.get_employee_ids()
            installer_names = self.db_querry.get_installer_names(employee_ids)
            ...
            selected_installers_id = self.db_querry.get_installers_id(selected_installers)

            task_id = self.db_querry.add_task(group_id, work_type_id, address_id, comment)
            self.db_querry.add_task_installers(task_id, selected_installers_id)
    ...
    async def close_task_message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
...
class CloseTaskBilling:
    def __init__(self):
        self.db_querry = DBQuerry()

    async def close_task_billing(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        keyboard = [
    ...
    async def close_billing_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        query = update.callback_query
        await query.answer()
        ...
        if query.data in ['closebilling_faults', 'closebilling_install_with_cable', 'closebilling_install_without_cable']:
            ...
            try:
                self.db_querry.update_task_status(task_id)
            except Exception as e:
...
if __name__ == '__main__':
    ...
    get_tasks = GetTasks()
    take_task = TakeTasks()
    close_task_monitoring = CloseTaskMonitoring() 
    close_task_billing = CloseTaskBilling() 

    application.add_handler(CommandHandler('get_tasks', get_tasks.get_task_main))
    application.add_handler(CallbackQueryHandler(get_tasks.get_task_handler))

    application.add_handler(CommandHandler('take_tasks', take_task.take_task))
    application.add_handler(CallbackQueryHandler(take_task.take_task_handler))

    application.add_handler(CommandHandler('close_task_monitoring', close_task_monitoring.close_task_monitoring))
    application.add_handler(CallbackQueryHandler(close_task_monitoring.close_monitoring_handler))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, close_task_monitoring.close_task_message_handler))

    application.add_handler(CommandHandler('close_task_billing', close_task_billing.close_task_billing))
    application.add_handler(CallbackQueryHandler(close_task_billing.close_billing_handler))
